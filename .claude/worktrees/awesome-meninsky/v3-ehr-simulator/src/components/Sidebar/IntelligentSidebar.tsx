...
const IntelligentSidebar: React.FC = () => {
    ...
    const handleValidation = async (data: ClinicalIntelligenceData) => {
        try {
            await validateClinicalData({
                code: data.mapped_icd10!,
                status: 'Confirmed',
                clinician_id: 'CLIN_099',
                timestamp: new Date().toISOString()
            });
            alert('Validated & Added to Chart');
        } catch (e) {
            alert('Clinical Data captured locally (API error).');
        }
    };
    ...
    return (
        <aside id="intelligence-sidebar" className="w-[30%] min-w-[350px] sticky top-0 h-screen overflow-y-auto bg-gray-50/80 backdrop-blur-xl border-l border-gray-200 z-50 flex flex-col p-6 shadow-[-4px_0_24px_-12px_rgba(0,0,0,0.1)]">
            <div className="mb-8">
                <h2 className="text-2xl font-black text-gray-900 tracking-tighter mb-2">INTELLIGENCE <span className="text-blue-600">HUB</span></h2>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-[0.2em]">Semantic Matching Engine V3.1</p>
            </div>
            ...
            <div className="flex-1">
                <div className="flex items-center justify-between mb-4 px-2">
                    <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest">Predictions</h3>
                    <span className="text-[10px] bg-blue-100 text-blue-700 font-bold px-2 py-0.5 rounded-full">{results.length} Matches</span>
                </div>
            ...
            <div className="mt-8 pt-8 border-t border-gray-200">
                <div className="flex items-center gap-3 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <div>
                        ...
                        <p className="text-gray-900 font-bold text-sm leading-none">Endpoint Secured</p>
                    </div>
                </div>
            </div>
        </aside>
