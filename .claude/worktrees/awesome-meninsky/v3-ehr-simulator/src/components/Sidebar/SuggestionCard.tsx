...
const SuggestionCard: React.FC<SuggestionCardProps> = ({ data, isLoading, onApprove }) => {
    ...
    if (!data) {
        return (
            <div className="p-6 bg-blue-50/30 rounded-lg border border-blue-200 border-dashed mb-4 flex flex-col items-center text-center">
                ...
                <p className="text-blue-700 text-sm">Try a different term or manual entry.</p>
            </div>
        );
    }
    ...
    return (
        <div className="group relative p-4 bg-white rounded-xl shadow-sm border border-gray-100 hover:border-blue-400 transition-all duration-200 mb-4 overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${data.hcc_data.hcc_impact ? 'bg-red-500' : 'bg-green-500'}`} />

            <div className="flex justify-between items-start mb-3">
                <div>
                    <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-1">Source</span>
                    <p className="text-gray-900 font-bold text-lg leading-tight">{data.original}</p>
                </div>
                {data.hcc_data.hcc_impact && (
                    <span className="bg-red-100 text-red-700 text-[10px] font-black px-2 py-0.5 rounded flex items-center gap-1 uppercase">HCC Risk</span>
                )}
            </div>

            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <span className="text-[10px] font-bold text-gray-500 block uppercase">ICD-10</span>
                    <span className="text-blue-600 font-black text-xl leading-none">{data.mapped_icd10 || 'N/A'}</span>
                </div>
                <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <span className="text-[10px] font-bold text-gray-500 block uppercase">Weight</span>
                    <span className="text-gray-900 font-black text-xl leading-none">{data.hcc_data.weight.toFixed(3)}</span>
                </div>
            </div>

            <button onClick={() => onApprove?.(data)} className="w-full h-12 bg-gray-900 hover:bg-blue-600 active:bg-blue-700 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 group-hover:scale-[1.01] transform active:scale-[0.98]">
